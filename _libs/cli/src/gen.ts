...
import { generateDts, generateEnvExample, generateJsonSchema, inspectSchema, loadDefinition } from "@confederation/core/index.js";
...
export async function gen(options: GenOptions): Promise<{ written: string[] }> {
    ...
    if (options.types) {
        artifacts.push({ name: "confederation-env.d.ts", content: generateDts(descriptors, { source }) });
    }
    if (options.example) {
        artifacts.push({ name: ".env.example", content: generateEnvExample(descriptors, { source }) });
    }
    if (options.json) {
        artifacts.push({ name: "confederation.schema.json", content: generateJsonSchema(definition.schema) });
    }
...
function resolveConfigPath(config: string | undefined, cwd: string): string {
    ...
    if (found === undefined) {
        throw new Error(`No confederation.config.* found at or above ${cwd}. Pass --config <path>.`);
    }
    return found;
}
...
function guardOverwrite(target: string, content: string, force: boolean): void {
    ...
    if (!firstLine.includes("Generated by confederation gen")) {
        throw new Error(`Refusing to overwrite ${target} (not a confederation-generated artifact). Pass --force to overwrite.`);
    }
}
