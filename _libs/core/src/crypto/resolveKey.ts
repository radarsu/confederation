...
export const PUBLIC_KEY_PATH = ".config/purenv-pub.key";
...
export function resolvePublicKey(cwd: string = process.cwd()): Uint8Array {
    ...
    if (!existsSync(path)) {
        throw new Error(`Public key not found at ${path}. Run \`purenv keygen\` to create one.`);
    }
...
export function resolvePrivateKey(options?: DecryptOptions): Uint8Array {
    ...
    const envInline = process.env["PURENV_PRIVATE_KEY"];
    if (envInline !== undefined && envInline !== "") {
        return base64urlDecode(envInline);
    }
    const envPath = process.env["PURENV_PRIVATE_KEY_FILE"];
    ...
    throw new Error(`No private key found. Set PURENV_PRIVATE_KEY env var, PURENV_PRIVATE_KEY_FILE, or place key at ${defaultPath}.`);
}

export function defaultPrivateKeyPath(cwd: string): string {
    const projectRoot = findProjectRoot(cwd);
    const name = readProjectName(projectRoot);
    return join(homedir(), ".config", "purenv", name, "private.key");
}
